        ...
        for ts in range(numwin):
            ...
            if TYPEFEATURE==1:
                ...
                if Imi > 0 and Imj > 0:
                    ...
                    simmap[ui][uj][ts] = jdVonMises(meani, mi, Imi[0]/math.pi/2.0, meanj, mj, Imj[0]/math.pi/2.0)
                else:
                    simmap[ui][uj][ts] = 0.0
            elif TYPEFEATURE==3:
                matrixi=datamap[ui][ts][0]
                matrixj=datamap[uj][ts][0]
                simmap[ui][uj][ts]=jdFuncForMclust(matrixi,matrixj)
